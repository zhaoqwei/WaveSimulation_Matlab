...
QP=100;QS=100;
c11i=c11/(QP*FM);c33i=c11i;c55i=c55/(QS*FM);c13i=c11i-2*c55i;
...
for	t=DT:DT:T
    ...
    vzzt=TZ.*imfilter(vz1,ddz1)/dt;
    vxxt=TX.*imfilter(vx1,ddx1)/dt;
    vxzt=TZ.*imfilter(vx1,ddz0)/dt;
    vzxt=TX.*imfilter(vz1,ddx0)/dt;
    taoxx1=taoxx0+c13.*TZ.*imfilter(vz1,ddz1)+c11.*TX.*imfilter(vx1,ddx1)+c13.*HZZ1*dt+c11.*HXX1*dt+c13i.*vzzt+c11i.*vxxt;
    taozz1=taozz0+c33.*TZ.*imfilter(vz1,ddz1)+c13.*TX.*imfilter(vx1,ddx1)+c33.*HZZ1*dt+c13.*HXX1*dt+c33i.*vzzt+c13i.*vxxt;
    taoxz1=taoxz0+c55.*TZ.*imfilter(vx1,ddz0)+c55.*TX.*imfilter(vz1,ddx0)+c55.*HXZ1*dt+c55.*HZX1*dt+c55i.*vxzt+c55i.*vzxt;
